...
describe('listVideos', () => {
    it('lists videos', async () => {
        const result = await listVideos({ limit: 5 })
        expect(result).toBeTruthy()
        console.log('Listed videos:', JSON.stringify(result, null, 2))
    })
})
...
describe('getUnsplashVideo', () => {
    it('gets video keyword', async () => {
        ...
        expect(result?.bestResultUrl).toMatchInlineSnapshot(
            `"https://videos.pexels.com/video-files/4550475/4550475-hd_720_1280_50fps.mp4"`,
        )
    })
})
